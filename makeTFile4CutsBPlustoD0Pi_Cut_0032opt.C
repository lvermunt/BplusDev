...
void makeInputAliAnalysisTaskSEBPlustoD0Pi(Bool_t bIsData = kFALSE){
  ...
  TString cutNumber = "Cut_0032opt";
  if(!bIsData) cutNumber+= "MC";
  //Centrality selection
  RDHFBPlustoD0Pi->SetUseCentrality(kFALSE);
  RDHFBPlustoD0Pi->SetMinCentrality(0);
  RDHFBPlustoD0Pi->SetMaxCentrality(100);
  ...
  if(bIsData){
    ...
    RDHFBPlustoD0Pi->SetOptPileup(AliRDHFCuts::kRejectPileupEvent);
